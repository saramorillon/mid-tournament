...
describe('createCallback', () => {
  ...
  it('should reply with missing end date message if end date is missing', async () => {
    ...
    await createCallback(interaction)
    expect(interaction.editReply).toHaveBeenCalledWith({ embeds: [missingEndDate()] })
  })

  it('should reply with invalid date format message if end date is not valid', async () => {
    const interaction = mockModalInteraction()
    mock(interaction.fields.getTextInputValue).mockReturnValueOnce('name').mockReturnValueOnce('end date')
    await createCallback(interaction)
    expect(interaction.editReply).toHaveBeenCalledWith({ embeds: [invalidEndDateFormat()] })
  })

  it('should reply with invlid date value message if end date is in the past', async () => {
    const interaction = mockModalInteraction()
    mock(interaction.fields.getTextInputValue).mockReturnValueOnce('name').mockReturnValueOnce('01/01/2000 01:00')
    await createCallback(interaction)
    expect(interaction.editReply).toHaveBeenCalledWith({ embeds: [invalidEndDateValue()] })
  })

  it('should reply with missing description message if description is missing', async () => {
    const interaction = mockModalInteraction()
    mock(interaction.fields.getTextInputValue).mockReturnValueOnce('name').mockReturnValueOnce('01/01/2023 01:00')
    await createCallback(interaction)
    expect(interaction.editReply).toHaveBeenCalledWith({ embeds: [missingDescription()] })
  })
